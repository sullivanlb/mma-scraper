...
async def insert_fighter(fighter_details: List[Dict], tapology_url: str, small_img_url: str) -> bool|int:
    """Process and insert or update fighter data with validation."""
    try:
        ...
        profile_hash = hashlib.sha256(unique_data.encode('utf-8')).hexdigest()
        ...
        fighter_data = {
            ...
            'tapology_url': tapology_url,  # Unique key for fighters
            'total_fights': total_fights,
            'hash': profile_hash,
        }

        new_fighter_id = await insert_data("fighters", [fighter_data], unique_key="tapology_url")
...
@RETRY_POLICY
async def insert_data(table: str, data: List[Dict], unique_key: str = None) -> Optional[str]:
    ...
    try:
        ...
        if unique_key:
            ...
            if existing_data.data:
                ...
                data_with_hash[0]['hash'] = data_hash
            ...
            # Insert new data with hash (no unique key provided)
            data_with_hash = data.copy()
            data_with_hash[0]['hash'] = data_hash
            response = supabase.table(table).insert(data_with_hash).execute()
...
async def process_event_url(event_url: str):
    ...
    try:
        ...
        event_data = {
            ...
            'broadcast': event_header.get('broadcast', ''),
            ...
            'tapology_url': event_url,  # Unique key for events
        }

        new_event_id = await insert_data("events", [event_data], unique_key="tapology_url")

        for fight in event_details[0]["Fight Card"]:
            try:
                # Insert or update Fighter 1
...
async def extract_event_urls(base_url: str) -> List[str]:
    ...
    page = 1
    max_retries = 3
    stop_pagination = False  # Flag to stop pagination

    # Make `now` and `seven_days_later` timezone-aware
    now = datetime.now(pytz.UTC)  # Use UTC or your desired timezone
    seven_days_later = now + timedelta(days=7)

    while not stop_pagination:  # Continue until the flag is set
        ...
        for attempt in range(max_retries):
            try:
                ...
                for event_url in extracted_urls:
                    ...
                    if event_details:
                        event_header = event_details[0]['Header'][0]
                        event_date = format_date(event_header['datetime'])
                        
                        # Ensure `event_datetime` is timezone-aware
                        event_datetime = datetime.fromisoformat(event_date)  # This is already timezone-aware

                        # Check if the event is in the past
                        if event_datetime < now:
                            logger.info(f"Event {event_url} is in the past. Stopping pagination after this page.")
                            stop_pagination = True  # Set the flag to stop pagination after this page
                            break

                        # Check if the event is within the next 7 days
                        if now <= event_datetime <= seven_days_later:
                            filtered_urls.append(event_url)

                event_urls.extend(filtered_urls)
                logger.info(f"Page {page} contained {len(filtered_urls)} URLs within the next 7 days")

                # Process events concurrently but limit concurrency
                processing_tasks = []
                for event_url in filtered_urls:
    ...
    return event_urls  # Return all collected event URLs
